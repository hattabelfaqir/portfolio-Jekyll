---
permalink: /projects/projet3/
---

**Description**  
Création d’une boutique GTA RP permettant l’achat de contenus *in-game*.  
Le projet a débuté sous la forme d’un site statique en HTML, CSS et JavaScript, avec pour objectif une évolution vers une version dynamique.

### Version 1 – Site statique
    
- Design réalisé sur Figma  
- Technologies : HTML, CSS, JavaScript  

**Maquette Figma (V1)**  
👉 [Voir la maquette](https://www.figma.com/design/P8rZmh94le5SMw1H8soeAq/trackmania-maquette--1-?node-id=0-1&t=1qh8HHU1pdKdu3np-1)

**Aperçus du site**

![Boutique V1 – Page 1]({{ site.baseurl }}/assets/boutiquev21.png),
![Boutique V1 – Page 2]({{ site.baseurl }}/assets/boutiquev22.png),
![Boutique V1 – Page 3]({{ site.baseurl }}/assets/boutiquev23.png),
![Boutique V1 – Page 4]({{ site.baseurl }}/assets/boutiquev24.png),
![Boutique V1 – Page 5]({{ site.baseurl }}/assets/boutiquev25.png),
![Boutique V1 – Page 6]({{ site.baseurl }}/assets/boutiquev26.png)

---

### Version 2 – Site dynamique (en cours)

- Nouvelle maquette orientée version dynamique  
- Technologies prévues :  
  - PHP  
  - Tailwind CSS  
  - JavaScript  

**Maquette Figma (V2)**  
👉 [Voir la maquette](https://www.figma.com/design/qqWd3o5nwSug2zLsPKXvSo/Sans-titre?node-id=0-1)

**Aperçus de la nouvelle version**

![Boutique V2 – Page 1]({{ site.baseurl }}/assets/boutiquev11.png),
![Boutique V2 – Page 2]({{ site.baseurl }}/assets/boutiquev12.png),
![Boutique V2 – Page 3]({{ site.baseurl }}/assets/boutiquev13.png)

**Statut**  
🛠️ Projet en cours de développement.
